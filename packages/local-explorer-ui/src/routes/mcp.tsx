	...
	return (
		<div className="flex h-full flex-col">
			...
			<div className="mx-auto flex w-full max-w-3xl flex-col gap-5 overflow-y-auto p-5">
					...
					<div className="flex flex-wrap gap-x-6 gap-y-3">
						{LOG_LEVELS.map((lvl) => (
							...
							>
								...
								/>
								<span className="text-kumo-default text-xs capitalize">
									{lvl}
								</span>
							</label>
						))}
				...
				>
					{resources.length === 0 ? (
						<p className="text-kumo-subtle text-xs italic">
						...
						<div className="flex flex-col gap-4">
							{(Object.keys(RESOURCE_META) as ResourceType[]).map((type) => {
								...
								return (
									<div key={type}>
										<div className="text-kumo-subtle mb-1.5 flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-wide">
											<Meta.icon className="h-3.5 w-3.5" />
											{Meta.title}
										</div>
										<div className="border-kumo-fill divide-kumo-fill divide-y rounded-md border">
											{items.map((r) => {
												...
												return (
													...
													>
														<span className="text-kumo-default font-mono text-xs">
															{r.label}
														</span>
														<Switch
															checked={allowed}
				...
				>
					{calls.length === 0 ? (
						<div className="flex flex-col items-center justify-center py-10 text-center">
							<ShieldCheckIcon size={26} className="text-kumo-subtle mb-2" />
							<p className="text-kumo-default text-xs font-medium">
								No agent activity yet
							</p>
							<p className="text-kumo-subtle mt-1 max-w-sm text-xs">
						...
						<div className="overflow-x-auto">
							<table className="w-full border-collapse text-sm">
								<thead>
									<tr className="text-kumo-subtle border-kumo-fill border-b text-left text-xs">
										...
										return (
											<Fragment key={key}>
												<tr
													...
													className="border-kumo-fill hover:bg-black/[0.03] dark:hover:bg-white/5 cursor-pointer border-b align-top"
												>
													<td className="text-kumo-subtle py-2 pl-1 text-xs">
														{isOpen ? (
													...
													<td className="text-kumo-subtle py-2 pr-3 font-mono text-[11px]">
														{c.created_at ?? ""}
													</td>
													<td className="text-kumo-default py-2 pr-3 font-mono text-xs">
														{c.tool ?? "-"}
													</td>
													<td className="text-kumo-subtle max-w-[18rem] truncate py-2 pr-3 font-mono text-[11px]">
														{c.args ?? ""}
													</td>
...
/** Absolute path to the stdio MCP server on this machine. */
const SERVER_PATH =
	"/Users/zin/Documents/workers-sdk-tail-experiment/experiments/wobs-local-traces/mcp-server/mcp-server.mjs";
...
function connectSnippet(agent: AgentId, explorerUrl: string): string {
	...
	switch (agent) {
		...
		case "claude":
			return `claude mcp add wobs-local -e WOBS_EXPLORER_URL=${explorerUrl} -- node ${SERVER_PATH}`;
		case "cursor":
			return JSON.stringify(
				{
					mcpServers: {
...
function cursorDeeplink(explorerUrl: string): string {
	const config = {
		command: "node",
		args: [SERVER_PATH],
...
function ConnectCard(): JSX.Element {
	const explorerUrl =
		typeof window !== "undefined" ? window.location.origin : "http://localhost:8799";
	const [agent, setAgent] = useState<AgentId>("opencode");
	const [copied, setCopied] = useState(false);

	const snippet = connectSnippet(agent, explorerUrl);
		...
		>
			<div className="flex flex-col gap-3">
				<div className="flex items-center gap-2">
					<div className="border-kumo-fill inline-flex rounded-md border p-0.5">
						{AGENTS.map((a) => (
							<button
								key={a.id}
								type="button"
					...
					{agent === "cursor" ? (
						<a
							href={cursorDeeplink(explorerUrl)}
				...
				<div className="relative">
					<pre className="border-kumo-fill bg-kumo-elevated text-kumo-default overflow-x-auto rounded-md border p-3 pr-10 font-mono text-[11px] leading-relaxed">
						{snippet}
					</pre>
					<button
						type="button"
						...
						className="text-kumo-subtle hover:bg-kumo-tint hover:text-kumo-default absolute right-2 top-2 rounded p-1"
					>
						{copied ? (
				...
				<p className="text-kumo-subtle text-[11px] leading-relaxed">
					{agent === "opencode"
					...
					Requires this dev server to be running. Adjust the path if your repo
					lives elsewhere.
				</p>
			</div>
		</Card>
...
function DetailBlock({
	...
	return (
		<div>
			<div className="text-kumo-subtle mb-1 text-[10px] font-medium uppercase tracking-wide">
				{label}
			</div>
			<pre className="border-kumo-fill bg-kumo-elevated text-kumo-default max-h-72 overflow-auto rounded-md border p-2.5 font-mono text-[11px] leading-relaxed">
				{prettyJson(value)}
			</pre>
		</div>
	);
...
function Card({
	...
	return (
		<section className="border-kumo-fill bg-kumo-base rounded-lg border p-4">
			<div className="mb-3 flex items-start justify-between gap-3">
				<div>
					<h3 className="text-kumo-default text-sm font-semibold">{title}</h3>
					{description ? (
						<p className="text-kumo-subtle mt-0.5 text-xs">{description}</p>
					) : null}
				</div>
				{action}
			</div>
